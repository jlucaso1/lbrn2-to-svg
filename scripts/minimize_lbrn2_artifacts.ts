...
const childElementsToRemoveFromCutSetting: string[] = [
  'minPower', 'maxPower', 'minPower2', 'maxPower2', 'speed', 'PPI',
  'JumpSpeed', 'wobbleStep', 'wobbleSize', 'wobbleEnable', 'numPasses',
  'perfLen', 'perfSkip', 'dotTime', 'bidir', 'crossHatch', 'overscan',
  'interval', 'angle', 'priority', 'tabCount', 'tabCountMax',
];
...
async function processLbrn2File(filePath: string): Promise<void> {
  ...
  try {
    ...
    if (!parsedObj.LightBurnProject) {
      console.warn(`  No LightBurnProject root found in ${filePath}. Skipping.`);
      return;
    }
